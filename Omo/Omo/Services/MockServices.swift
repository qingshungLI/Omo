...
final class AppStore: ObservableObject {
    ...
    private static var shouldUseV2Root: Bool {
        ...
        return !arguments.contains("-OmoUseLegacyRoot")
            && !arguments.contains("-RecalloUseLegacyRoot")
            && !arguments.contains("-ShibeiUseLegacyRoot")
            && environment["OMO_USE_LEGACY_ROOT"] != "1"
            && environment["RECALLO_USE_LEGACY_ROOT"] != "1"
    ...
    func createChapter(from input: String) async -> Bool {
        ...
        #if DEBUG
        print("[Omo] AppStore.createChapter start target=\(targetMode.rawValue), sourceType=\(parsedInput.sourceType.rawValue), canSubmit=\(parsedInput.canSubmit), device=\(anonymousDeviceId.suffix(6))")
        #endif
        ...
        do {
            ...
            case .localAPI, .cloudAPI:
                guard let client = apiClient(for: targetMode) else {
                    ...
                    #if DEBUG
                    print("[Omo] AppStore.createChapter missing valid API client for target=\(targetMode.rawValue), cloudURL=\(cloudAPIBaseURLString)")
                    #endif
                    return false
                }
                ...
                #if DEBUG
                print("[Omo] AppStore.createChapter accepted chapter=\(created.chapter.id), status=\(created.chapter.status.rawValue)")
                #endif
            }
            ...
            #if DEBUG
            print("[Omo] AppStore.createChapter failed target=\(targetMode.rawValue), error=\(error.localizedDescription)")
            #endif
            return false
        }
    }
