...
struct APIClient {
    ...
    private static var launchArgumentBaseURL: URL? {
        ...
        if let value = ProcessInfo.processInfo.environment["OMO_API_BASE_URL"]
            ?? ProcessInfo.processInfo.environment["OMO_V2_API_BASE_URL"]
            ?? ProcessInfo.processInfo.environment["RECALLO_API_BASE_URL"]
    ...
    private func get<Response: Decodable>(_ path: String) async throws -> Response {
        ...
        #if DEBUG
        print("[Omo] API GET \(url.absoluteString) device=\(deviceId.suffix(6))")
        #endif
        ...
        #if DEBUG
        print("[Omo] API GET status=\(httpResponse.statusCode) path=\(path)")
        #endif
    ...
    private func send<Request: Encodable, Response: Decodable>(
        ...
        request.setValue(deviceId, forHTTPHeaderField: "X-Device-Id")
        #if DEBUG
        print("[Omo] API \(method) \(url.absoluteString) device=\(deviceId.suffix(6))")
        #endif
        if method != "DELETE" {
        ...
        #if DEBUG
        print("[Omo] API \(method) status=\(httpResponse.statusCode) path=\(path)")
        #endif
    ...
    private func decode<Response: Decodable>(_ type: Response.Type, from data: Data, path: String) throws -> Response {
        do {
            ...
            #if DEBUG
            print("[Omo] API decode failed path=\(path): \(message)")
            #endif
            throw APIClientError.decoding(message)
        } catch {
            #if DEBUG
            print("[Omo] API decode failed path=\(path): \(error.localizedDescription)")
            #endif
            throw error
        }
    }
...
enum PushTokenEnvironment: String, Codable {
    ...
    static var current: PushTokenEnvironment {
        let value =
            Bundle.main.object(forInfoDictionaryKey: "OmoAPNSEnvironment") as? String
            ?? Bundle.main.object(forInfoDictionaryKey: "ShiBeiAPNSEnvironment") as? String
