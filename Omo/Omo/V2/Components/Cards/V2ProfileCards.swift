...
private enum V2ProfileSettingsSheet: String, Identifiable {
    ...
    var paragraphs: [String] {
        ...
        case .notifications:
            [
                "哦莫 会在内容生成完成或失败时发送系统通知，帮助你回到对应章节继续学习。",
                "如果你没有开启系统通知，仍然可以在 App 内通知页查看生成结果。成功通知打开后会自动归档，失败通知会保留到你处理或手动移除。"
            ]
        case .privacy:
            [
                "你提交的文字、文章链接和生成结果会发送到 哦莫 云端，用于提取知识点、生成题目和保存学习进度。",
                "生成过程中，内容可能会被发送给第三方 AI 模型服务处理。哦莫 不会把你的内容公开展示给其他用户。",
                "首次使用真实生成前，哦莫 会要求你确认 AI 处理说明；确认后这项说明可以在这里回看。",
                "服务器会保存章节、题目、通知、学习记录、收藏、生成额度和必要诊断信息。你可以在“我的”页删除当前匿名设备下的数据。"
            ]
        case .account:
            [
                "你可以继续匿名使用 哦莫，也可以选择绑定 Apple 账号。",
...
private struct V2ProfileNotificationPermissionPanel: View {
    ...
    private var statusDescription: String {
        switch status {
        case .authorized, .provisional, .ephemeral:
            "生成完成或失败后，哦莫 可以通过系统通知提醒你。"
        case .denied:
            "你已经在系统里关闭了通知。需要到 iOS 设置中重新允许 哦莫 发送通知。"
        case .notDetermined:
