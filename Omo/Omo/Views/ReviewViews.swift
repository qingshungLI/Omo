            ...
            SBCard(padding: 22) {
                VStack(spacing: 12) {
                    Image(systemName: "star")
                        ...
                        .background(OmoTheme.yellow.opacity(0.35))
                        .clipShape(Circle())
    ...
    var body: some View {
        Button {
            ...
            Image(systemName: store.isFavoriteQuestion(question) ? "star.fill" : "star")
                ...
                .foregroundStyle(store.isFavoriteQuestion(question) ? OmoTheme.yellow : OmoTheme.text)
                .frame(width: 42, height: 42)
                .contentShape(Rectangle())
        }
        .buttonStyle(.plain)
        ...
        Button(action: action) {
            HStack(alignment: .top, spacing: 12) {
                Text(isCorrect ? "✓" : option.id)
                    .font(.system(size: 18, weight: .bold))
                    .foregroundStyle(isCorrect ? OmoTheme.onPrimary : OmoTheme.primary)
                    .frame(width: 26, height: 26)
                    .background(isCorrect ? OmoTheme.success : .clear)
                    .clipShape(Circle())
                Text(option.text)
                    .font(.system(size: 16, weight: .medium))
                    .foregroundStyle(isWrong ? OmoTheme.answerWrongText : OmoTheme.text)
                    .multilineTextAlignment(.leading)
                    .lineSpacing(3)
                    .frame(maxWidth: .infinity, alignment: .leading)
                    .fixedSize(horizontal: false, vertical: true)
            ...
            .background(isCorrect ? OmoTheme.successBackground : isWrong ? OmoTheme.dangerBackground : OmoTheme.card)
            .overlay(
                RoundedRectangle(cornerRadius: OmoTheme.radius, style: .continuous)
                    .stroke(isCorrect ? OmoTheme.success.opacity(0.35) : isWrong ? OmoTheme.error.opacity(0.35) : OmoTheme.line, lineWidth: 1)
            )
        ...
        AppScaffold(store: store, title: "", showsTopBar: false, showsTabBar: false) {
            if let question {
                VStack(spacing: 0) {
                    ...
                    .padding(.bottom, 30)
                    .background(
                        LinearGradient(colors: [OmoTheme.surface.opacity(0), OmoTheme.surface], startPoint: .top, endPoint: .bottom)
                    )
                }
            }
        }
...
private struct ExplanationCard: View {
    ...
    var body: some View {
        ...
        .background(warm ? OmoTheme.sourceBackground : .clear)
    }
