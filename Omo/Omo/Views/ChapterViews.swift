        ...
        return Button {
            ...
            VStack(spacing: 6) {
                Text(title)
                    ...
                    .foregroundStyle(isSelected ? OmoTheme.text : OmoTheme.faint)
                Capsule()
                    .fill(isSelected ? OmoTheme.text : Color.clear)
...
private enum ChapterReviewDisplayStatus {
    ...
    var background: Color {
        switch self {
        case .waiting:
            OmoTheme.reviewWaitingBackground
        case .inProgress:
            OmoTheme.reviewInProgressBackground
        case .completed:
            OmoTheme.reviewCompletedBackground
        }
    }
}
                            ...
                            Button {
                                ...
                                SBCard {
                                    HStack(spacing: 8) {
                                        ...
                                        if notification.read {
                                            Text(store.localized("notifications.read"))
                                                ...
                                                .foregroundStyle(OmoTheme.muted.opacity(0.72))
                                        }
                                    }
                                    Text(store.chapters.first(where: { $0.id == notification.chapterId })?.title ?? notification.localizedTitle(language: store.appLanguage))
                                        ...
                                        .foregroundStyle(notification.read ? OmoTheme.muted : OmoTheme.text)
        ...
        SBCard(padding: 18) {
            HStack(alignment: .top, spacing: 12) {
                ...
                Image(systemName: "chevron.right")
                    .font(.system(size: 12, weight: .semibold))
                    .foregroundStyle(OmoTheme.faint)
            }
        }
    }
}
        ...
        SBCard {
            VStack(spacing: 12) {
                ...
                Text(chapter.visibleStatusText(language: store.appLanguage))
                    ...
                    .foregroundStyle(OmoTheme.error)
        ...
        Text(block.text)
            ...
            .background(
                RoundedRectangle(cornerRadius: 12, style: .continuous)
                    .fill(isFocused ? OmoTheme.inputFocusBackground : .clear)
            )
            .overlay(
                RoundedRectangle(cornerRadius: 12, style: .continuous)
                    .stroke(isFocused ? OmoTheme.yellow.opacity(0.8) : .clear, lineWidth: 1)
            )
    }
}
