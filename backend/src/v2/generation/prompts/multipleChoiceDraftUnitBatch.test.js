...
test("rejects unit MC core drafts that include option fields", () => {
  const result = validateMultipleChoiceDraftUnitBatchOutput(
    {
      unitId: "u1",
      questions: [
  ...
  assert.equal(result.ok, false);
  assert.match(result.errors.join("\n"), /options must be generated by multipleChoiceOptionSetUnitBatch/);
  assert.match(result.errors.join("\n"), /correctOptionId must be generated by multipleChoiceOptionSetUnitBatch/);
});
