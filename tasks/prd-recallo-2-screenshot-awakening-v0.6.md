...
1. 首页收敛为“今日 / 知识库 / 我的”三 Tab；今日默认单张召回并保留连续召回；知识库只展示真实卡片，不伪造知识图谱。
...
4. **首页只有“今日 / 知识库 / 我的”三 Tab。** 今日是默认落点，默认单张召回，也保留连续召回；知识库只展示用户真实拥有的卡片与记忆碎片，不伪造知识图谱或关系网络。
5. **单张与连续召回都是正式能力。** 用户可选“召回 1 张”或“连续召回”；连续模式至多 10 张，任何一张之后都可退出。
...
| Tab | 内容 | 明确不做 |
| --- | --- | --- |
| 今日 | 默认落点。记忆池选择、单张召回（默认）、连续召回、新截图入口、毛球陪伴 | 不堆砌运营位、 banner 和任务系统 |
...
`MemoryAssessment` 三档对应界面文案“记得 / 模糊 / 忘记”；`RecallTaskKind` 中 `masked_semantic` 只用于首次唤醒，`judgment` / `choice` 用于后续复现。
...
- 今日 / 知识库 / 我的三 Tab 首页；
- 三类透明记忆池、单张与连续召回；
- 可跳过召回过场（约 1450ms）和 R / SR / SSR 价值光迹；
- 首次精确语义遮挡的擦开 / 点击揭示，后续判断 / 选择变式；
- 记得 / 模糊 / 忘记三档反馈与简单调度；
- 封存 / 唤醒 / 稳固 / 铭刻状态与个人记忆收藏册入册反馈；
- 毛球原创角色与克制动效，prefers-reduced-motion 全链路降级；
- 卡片、截图与账号删除。
