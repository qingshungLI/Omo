...
开始召回后，界面使用一个可跳过、可打断的短过场，把调度器已经选定的旧内容卡从个人记忆中“取回”。首张完整过场在 1250ms 时完成稀有度落定并进入主动回忆：

```text
上拖卡叠 / 点击等价按钮
→ 0–120ms：入口轻压蓄力，毛球转头
→ 120–480ms：毛球翻找并取卡，卡片升起（倾斜不超过 ±4°）
→ 480–950ms：毛线光迹环绕，表达已确定的知识潜力
→ 950–1250ms：卡框、材质与 R / SR / SSR 徽标落定
→ 1250ms：出现主动回忆提示
```

用户在检查点选择继续后，后续卡使用约 700ms 短过场，只保留毛球取卡、卡片升起、停稳与提示，不重复完整光迹仪式。
...
- 首张在 1250ms 完成稀有度落定、后续约 700ms 的可跳过召回过场和 R / SR / SSR 价值光迹；
...
- 同一使用会话优先不重复卡版本；
- 首张在 1250ms 完成稀有度落定并进入主动回忆，后续卡过场约 700ms；二者均可跳过、可打断，倾斜不超过 ±4°；
