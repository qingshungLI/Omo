...
> 日期：2026-07-25
>
> 实现验证 SHA：`ebb78b228615c224b1fb427de64da47cb42fc767`
>
> 验证环境：`bridge-amax` 远程集成工作树

## 1. 结论边界

本轮完成的是 Step 0 代码侧安全门与 Step 1A 持久化纵切片的同一 SHA 集成验证。确定性测试和静态守卫通过，但这不等于生产迁移、真实平台链路或 iOS 运行时已经验收。
...
- 新增单卡删除合同，并将卡片数据纳入设备删除与账号删除事务级联；
- iOS 已对齐列表、assessment 和删除合同，识别 `create_card / archive_only / needs_confirmation`，并在客户端抽取池排除碎片和待确认项；
- 稀有度与 mastery 保持独立，不把 R/SR/SSR 当作掌握程度。

## 3. 同一 SHA 验证结果

以下四组结果均针对实现验证 SHA `ebb78b228615c224b1fb427de64da47cb42fc767`，没有混用其他代码版本：

| 验证面 | 入口 | 结果 | 能证明什么 |
| --- | --- | ---: | --- |
| Security | current secret scan 与 security contract tests | 14 / 14 PASS | 当前树、工作流合同和允许工作树策略通过代码侧检查 |
| Backend | `npm --prefix backend run check` | 163 / 163 PASS | 迁移 runner、Repository、幂等、删除、现有后端回归通过 |
| iOS guard | `npm run check:ios` | 8 / 8 PASS | iOS 生产静态守卫和合同引用通过 |
| UI guard | `node tools/v2-ui-regression-guard.mjs` | 27 / 27 PASS | 当前 V2 UI 关键结构与片段排除规则通过静态回归 |
...
- **Gate B：未通过。** 仍缺 live migration、任务恢复、数据库断连、并发提交和删除竞争测试。

## 5. 下一步门槛

1. 完成外部 key 轮换、旧 key 失效验证和 Git 历史清理，关闭 M0 阻断；
2. 在明确授权的非生产 Postgres 上执行空库与升级迁移、CRUD、级联和并发幂等 smoke；
3. 建设 durable worker 后执行重启、重试、dead-letter 与删除竞争测试；
