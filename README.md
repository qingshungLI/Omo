...
当前生产链路没有 Claude。截图流支持 B站、抖音和小红书：Qwen 视觉模型直接读取原图中的平台、标题、作者、内容类型和播放位置；TikHub 只在对应平台核对来源。视频优先使用字幕、ASR 兜底，小红书图文则读取公开正文，再生成截图附近的记忆卡。
...
本地截图流程演示页是 `http://127.0.0.1:5173/demo`。上传原图后会调用同一个 `POST /api/sources/image-flow` 接口；图片直接发送给配置的 Qwen 视觉模型，不经过 Apple Vision 或 Paddle OCR。平台已知时只搜索该平台；平台不明时才在已启用 adapter 间受控搜索。没有找到标题和作者均可信且无歧义的结果时，流程会停止，不会生成错误卡片。

如果只想查看当前 iOS 产品效果，可打开 `http://127.0.0.1:5173/app-demo`。这是独立的响应式交互预览，覆盖截图导入、分析过渡、单张或连续抽卡、拖拽揭晓和记忆自评；它不会上传图片或调用模型，也不属于正式 Web 客户端。
